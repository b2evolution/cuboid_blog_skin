...
echo $params['time_after'];

$commented_Item = & $Comment->get_Item();
$Comment->edit_link( ' / ', '', '#', '#', 'edit_link', '&amp;', true, rawurlencode( $Comment->get_permanent_url() ) ); /* Link to backoffice for editing */
$Comment->delete_link( ' / ', '', '#', '#', 'delete_link', false, '&amp;', true, false, '#', rawurlencode( $commented_Item->get_permanent_url() ) ); /* Link to backoffice for deleting */

echo $params['comment_info_after'];

// Avatar:
echo $params['comment_avatar_before'];
$Comment->avatar( 'crop-128x128', 'avatar' );
